...
if [ -e $(dirname $0)/images/platform_security_patch.txt ]; then
PACKAGE_SECURITY_PATCH_LEVEL_VER=`cat $(dirname $0)/images/platform_security_patch.txt`
SECURITY_PATCH_LEVEL_VER=$(date -d "$PACKAGE_SECURITY_PATCH_LEVEL_VER" +%s);
fi
ver_string=`fastboot $* getvar security-patch-level 2>&1 | grep -oP "security-patch-level: \K\d+(-\d+)*"`
if [ -z "$ver_string" ]; then ver=0;
else
ver=$(date -d "$ver_string" +%s);
fi
if [ $ver -gt $SECURITY_PATCH_LEVEL_VER ]; then
echo "Current device security patch level version($ver_string) is greater than this pakcage($PACKAGE_SECURITY_PATCH_LEVEL_VER)";
exit 1;
fi
