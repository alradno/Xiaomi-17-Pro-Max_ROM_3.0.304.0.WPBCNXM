fastboot %* getvar product 2>&1 | findstr /r /c:"^product: *popsicle" || echo Missmatching image and device
fastboot %* getvar product 2>&1 | findstr /r /c:"^product: *popsicle" || exit /B 1
...
if exist "%~dp0images\platform_security_patch.txt" (for /f "usebackq" %%a in ("%~dp0images\platform_security_patch.txt") do (set PACKAGE_SECURITY_PATCH_LEVEL_VER=%%a))
for /f "delims=" %%a in ('powershell -Command "$dateString = '%PACKAGE_SECURITY_PATCH_LEVEL_VER%'; $date = Get-Date $dateString; ($date - (Get-Date '1970-01-01')).TotalSeconds"') do set "SECURITY_PATCH_LEVEL_VER=%%a"
for /f "tokens=2 delims=: " %%i in ('fastboot %* getvar security-patch-level 2^>^&1 ^| findstr /r /c:"security-patch-level:"') do (set ver_string=%%i)
if [%ver_string%] EQU [] (
    set ver=0
) else (
    for /f "delims=" %%a in ('powershell -Command "$dateString = '%ver_string%'; $date = Get-Date $dateString; ($date - (Get-Date '1970-01-01')).TotalSeconds"') do set "ver=%%a"
)
set secpatchlevelcheck="security patch level check pass"
if %ver% GTR %SECURITY_PATCH_LEVEL_VER% set secpatchlevelcheck="Current device security patch level version(%ver_string%)) is greater than this pakcage(%PACKAGE_SECURITY_PATCH_LEVEL_VER%)"
echo %secpatchlevelcheck% | findstr /r /c:"pass" || @echo "secpatchlevelcheck check error" && exit /B 1

for /f "tokens=2" %%i in ('fastboot %* getvar dpstatus 2^>^&1') do (
    if "%%i"=="0x20" (
        fastboot %* flash apdp %~dp0images/apdp_minidump.mbn || @echo "Flash apdp error" && exit /B 1
        fastboot %* flash apdpb %~dp0images/apdp_minidump.mbn || @echo "Flash apdpb error" && exit /B 1
    )
)
...
fastboot %* flash xbl_ab %~dp0images/xbl_sc.elf || @echo "Flash xbl error" && exit 1
...
fastboot %* flash tme_seq_patch_ab %~dp0images/sequencer_ram.elf || @echo "Flash tme_seq_patch error" && exit 1
fastboot %* flash tme_fw_ab %~dp0images/signed_firmware_soc_view.elf || @echo "Flash tme_fw error" && exit 1
fastboot %* flash tme_config_ab %~dp0images/tme_config.elf || @echo "Flash tme_config error" && exit 1
fastboot %* flash xbl_ac_config_ab %~dp0images/xbl_ac_config.mbn || @echo "Flash xbl_ac_config error" && exit 1
fastboot %* flash ddr %~dp0images/zeros_5sectors.bin || @echo "Flash ddr error" && exit 1
...
fastboot %* flash spuservice_ab %~dp0images/spu_service.mbn || @echo "Flash spuservice error" && exit 1
...
fastboot %* flash soccp_ab %~dp0images/soccp.bin || @echo "Flash soccp error" && exit 1
...
fastboot %* flash tz_ac_config_ab %~dp0images/tz_ac_config.mbn || @echo "Flash tz_ac_config error" && exit 1
fastboot %* flash hyp_ac_config_ab %~dp0images/hyp_ac_config.mbn || @echo "Flash hyp_ac_config error" && exit 1
fastboot %* flash dcp_ab %~dp0images/dcp.bin || @echo "Flash dcp error" && exit 1
fastboot %* flash tz_qti_config_ab %~dp0images/tz_qti_config.mbn || @echo "Flash tz_qti_config error" && exit 1
fastboot %* flash secretkeeper_ab %~dp0images/secretkeeper.mbn || @echo "Flash secretkeeper error" && exit 1
...
fastboot %* flash qtvm_dtbo_ab %~dp0images/qtvm-dtbo.img || @echo "Flash qtvm_dtbo error" && exit 1
